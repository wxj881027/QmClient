...
已完成清单（带证据）、按接手人分类的待办（等 #264 合入后 rebase / 需拍板 4 项 / 必须实机 3 项）、
两个既有红的归属、常用命令、以及 6 条踩坑记录（worktree junction、clang-format 20/22、
GitHub 删分支连带关 PR、先 fetch、指标口径、新 worktree 的 submodule junction）。

**当前收尾状态**：同步基线、切片流程、逐轮决策与证据链条都已建立并跑通（三段 PR + 91 个提交 +
四组自动化测试 + 84 例工具单测）；与上游的内容覆盖率从 42.0% 提到 44.6%（实追 88 条）。
**未完成的是「推进」本身**：三段 PR 待合并、`#264` 合入后需再 rebase 一次确认 CI 无红项、
按区域手工融合（editor / game-client）尚未开工 —— 这些都需要维护者拍板或实机配合，
故本轮不宣告目标完成，交接说明里逐条列明。
...
### 1. 回退自审（`upstream_revert_audit.py`）

```
链上带 -x 溯源的提交：65 条
已被上游回退且链上还没回退：0 条（需要处理）
已被上游回退、但链上已包含回退：1 条（无需处理）  链上 b053c740ad ← 上游 96d35b3f94（libpng）
曾回退但又被回退（净效果保留）：1 条（无需处理）  链上 c7e19c415e ← 上游 8edc5a3c21（tune zone）
```

`-x` 条目从 61 增到 65（新增的官方语义 cherry-pick 等带 `-x`）。**0 条需要处理** ——
重排没有破坏溯源，也没有把「上游已回退的改动」留在链上。

### 2. 基线 / 冲突快照（`upstream_status.py --conflicts`，新基线之上）

| 指标 | 本次 | 同步开始时 |
|------|------|------------|
| 领先 / 落后 | 3701 / 2117 | 3600 / 2117 |
| 本地改动文件 | 1684 | 1656 |
| 上游改动文件 | 784 | 784 |
| 双方都改过 | 592 | 569 |
| 上游非 merge 提交 | 1258（重叠 1210） | 1258 |
| **零重叠（可直接摘）** | **48** | 71 |
| 干跑合并冲突路径 | 334 | 330 |

冲突类型：content 295、add/add 28、modify/delete 10、submodule 1、rename/delete 1；
最集中：`src/game/client` 64、`src/engine/shared` 32、`src/game/editor` 30、`src/engine/client` 26、`src/game/server` 20。

**读法**：`落后` 仍是 2117（窗口起点未变），`领先` 从 3600 增到 3701 是同步链自己加的提交。
真正说明「还能直接摘多少」的是**零重叠可摘：71 → 48**，且冲突集中在本地重写区域 ——
与 S22「收益递减」、S31「缺口热点在 client/editor」一致：**继续扫小提交的空间只剩 48 条，
下一步该做按区域手工融合而不是继续 cherry-pick**。

### 3. CI 状态（同一时点）

- PR-1/PR-2/PR-3 的 `check-style` / Linux / macOS 全绿，PR-2/PR-3 的 Windows 也绿；
...
| Analyze (python) | PASS | — | — | — |
| Analyze (cpp) | 待跑 | — | — | — |
| `check-clang-tidy` | 待跑 | 待跑 | 待跑 | **既有**（见下） |
| `check-clang-san` | 待跑 | 待跑 | FAIL（19m1s） | **既有** |

**关键结果**：PR-2 上一轮红的两处（Linux 的 `emote 99999999999999999999` 用例、macOS 的
`QmRoundedRect` flaky）现在**都转绿** —— 由 master 自己在 PR #256 里的修复带来，
印证了 S37 的判断（那两处确实是 master 级别的既有问题，修在 master 上比修在 PR 里更合适）。

`check-clang-tidy` 与 `check-clang-san` 仍是既有红：master 在 PR #256 合并**之后**的两次 push 运行
（`07:01`、`13:14`）结论都是 `failure`，与我的链无关（按维护者决定保持原样）。

### 差距量化在新基线上重测：数字与上一轮**完全相同**

| 引用 | 逐行命中 | ABSENT | ALREADY-PRESENT | PARTIAL |
|------|----------|--------|-----------------|---------|
| 新基线 `7cf432eacb` | 23172/55108 = **42.0%** | 522 | 368 | 280 |
| 链尾 `3f6b1fe9cc` | 24552/55108 = **44.6%** | 434 | 453 | 283 |

本轮实际追上 **88 条**，与 S31 在旧基线上测得的值逐位一致。原因很直白：master 新增的 5 个提交
是**本地**测试/CI 修复（`str_toint` 范围检查、浮点断言、rust 测试参数），**不含上游内容行**，
所以这个「与上游的内容差距」指标不动。这也顺便说明该指标的适用范围：它衡量的是**上游内容的覆盖**，
对本地修复/重构不敏感（S18 已记的两条低估口径之外，再加这一条观察）。
