...
| qm.speedrun.hud | src/game/client/components/hud.cpp | 在官方 HUD 可见性边界内提供 Qm speedrun HUD 绘制槽位 | Qm_ADAPTER | Qm 计时器关闭或 `cl_showhud` 关闭时不增加绘制；官方 HUD、输入和网络行为不变 | HUD 绘制顺序、视频录制 HUD 可见性和后续官方 HUD 重构 | 官方提供可插拔 HUD overlay 槽位；旧 speedrun presentation 完全迁移后 | Release build; C++ tests; runtime smoke; real race pending | active |
...
| qm.test.hidden_window | src/engine/client/backend_sdl.cpp; scripts/integration_test.py; qmclient_scripts/check_qmclient_runtime_smoke.py | 集成 smoke 在 Windows 上创建客户端窗口时保持隐藏，避免测试弹出可见游戏窗口 | Qm_ADAPTER | 未设置 `DDNET_TEST_HIDE_WINDOW=1` 时完全不改变窗口行为；设置后只影响测试进程的初始窗口可见性 | SDL 窗口创建参数、Windows 测试进程环境和未来 SDL3 迁移 | 官方测试框架提供原生 headless/window-hidden 能力，且不再需要该环境变量触点 | Release build; runtime smoke; client/server integration smoke | active |
