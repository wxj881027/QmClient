...
- backend 选择完成后增加 `graphics.backend_selection` 旁路事件，记录选择出的 backend 和 `selection_source=config|environment`；Qm report 同时保留初始化开始时的配置请求、最近一次 graphics info 的 backend 配置、选择来源和实际 active backend。`requested_backend_config` 暂作为与 `requested_backend` 同值的兼容字段保留；环境变量覆盖时不把配置差异误判为 fallback。
