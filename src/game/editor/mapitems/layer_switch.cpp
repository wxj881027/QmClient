...
bool CLayerSwitch::HasContentInRect(const CIntRect &Rect) const
{
	// 开关层把编号/延时单独存放，图块数组为空时也可能有数据
	return CLayerTiles::HasContentInRect(Rect) || AnyInRect(m_Width, m_Height, Rect.x, Rect.y, Rect.w, Rect.h, [this](int x, int y) { return m_pSwitchTile[y * m_Width + x].m_Type != 0; });
}
