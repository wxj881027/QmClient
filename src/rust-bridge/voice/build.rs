...
/// 需要先运行 setup-opus.ps1 生成 MSVC 导入库。
fn check_opus_prebuilt() {
    let manifest_dir = env::var("CARGO_MANIFEST_DIR").unwrap();
    let manifest_path = PathBuf::from(&manifest_dir);
    let opus_lib_dir = manifest_path.join("opus-prebuilt").join("lib");

    if !opus_lib_dir.exists() {
        println!(
            "cargo:warning=opus 预编译库目录不存在: {}",
            opus_lib_dir.display()
        );
        println!("cargo:warning=请先运行设置脚本: pwsh -File qmclient_scripts/setup-opus.ps1");
    }
}
...
fn configure_sdl2() {
    ...
    let (lib_dir, include_dir) = if target.contains("windows") {
        if target.contains("x86_64") {
            (
                ddnet_libs.join("windows").join("lib64"),
                ddnet_libs.join("include").join("windows"),
            )
        } else if target.contains("i686") {
            (
                ddnet_libs.join("windows").join("lib32"),
                ddnet_libs.join("include").join("windows"),
            )
        } else if target.contains("aarch64") {
            (
                ddnet_libs.join("windows").join("libarm64"),
                ddnet_libs.join("include").join("windows"),
            )
        } else {
        ...
        if target.contains("x86_64") {
            (
                ddnet_libs.join("linux").join("lib64"),
                ddnet_libs.join("include"),
            )
        } else if target.contains("i686") {
            (
                ddnet_libs.join("linux").join("lib32"),
                ddnet_libs.join("include"),
            )
        } else {
            // 对于其他 Linux 目标，使用系统库
        ...
        if target.contains("x86_64") {
            (
                ddnet_libs.join("mac").join("lib64"),
                ddnet_libs.join("include").join("mac"),
            )
        } else if target.contains("aarch64") {
            (
                ddnet_libs.join("mac").join("libarm64"),
                ddnet_libs.join("include").join("mac"),
            )
        } else {
            (
                ddnet_libs.join("mac").join("libfat"),
                ddnet_libs.join("include").join("mac"),
            )
        }
        ...
        if target.contains("aarch64") {
            (
                ddnet_libs.join("android").join("libarm64"),
                ddnet_libs.join("include").join("android"),
            )
        } else if target.contains("armv7") {
            (
                ddnet_libs.join("android").join("libarm"),
                ddnet_libs.join("include").join("android"),
            )
        } else if target.contains("x86_64") {
            (
                ddnet_libs.join("android").join("lib64"),
                ddnet_libs.join("include").join("android"),
            )
        } else if target.contains("i686") {
            (
                ddnet_libs.join("android").join("lib32"),
                ddnet_libs.join("include").join("android"),
            )
        } else {
        ...
        println!(
            "cargo:warning=Unsupported target for SDL2: {}, using system library",
            target
        );
        println!("cargo:rustc-link-lib=SDL2");
        return;
    ...
    if lib_dir.exists() {
        ...
        println!(
            "cargo:warning=SDL2 lib directory not found: {}",
            lib_dir.display()
        );
