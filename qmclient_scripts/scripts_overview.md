...
- `qmclient_scripts/support/upstream_merge_survey.py`：手工融合的排序依据（只读）；按区域统计「双方都改过」的文件，给出每个文件的双方非 merge 提交数与增删行数，按「双方都动得多」排序 —— 用来挑融合性价比最高的文件。例：`py -3 qmclient_scripts/support/upstream_merge_survey.py --repo tmp/sync-slice-1 --path src/game/client --top 20`。方案见 `docs/development/upstream-manual-merge-plan.md`
